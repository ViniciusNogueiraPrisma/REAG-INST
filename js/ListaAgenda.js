$(document).ready(function () {
  ...
  setTimeout(
    "$('div#eventos div').each(function () {var id = $(this).attr('id');var count = $(this).find('ul').children().length;var titulo = $(this).find('a#titleEvento').text().toLowerCase();var dataFim = '0';if($(this).attr('datafim') != ''){dataFim = $(this).attr('datafim').split('/')[0];}$('.ui-datepicker-calendar a').each(function () {var dia = $(this).text();if (dia == id || dia == dataFim) {if (titulo.indexOf('call') != -1 || titulo.indexOf('conferência') != -1 || titulo.indexOf('conference') != -1) {$(this).addClass('blue');} else if (titulo.indexOf('divulgação') != -1 || titulo.indexOf('release') != -1 || titulo.indexOf('results') != -1) {$(this).addClass('yellow');} else if (titulo.indexOf('webcast') != -1) {$(this).addClass('pink');} else {$(this).addClass('green');}}});});",
    1000
  );
});
...
function excluirEventoDaListEnviarLembrete(e) {
  var excluirId =
    e.target.id == "spanBtnExcluir"
      ? e.target.parentElement.attributes.idevento.value
      : e.target.attributes.idevento.value;

  var splitCookieEvento = docCookies
    .getItem("CookieListaAgendaREAG")
    .split(",");
  ...
  var splitCookieLembrete = docCookies
    .getItem("CookieEnviarLembreteAgendaREAG")
    .split(",");

  for (var i = 0; i < splitCookieLembrete.length; i++) {
    if (splitCookieLembrete[i].split("&D")[0] == excluirId) {
      splitCookieLembrete = $.grep(splitCookieLembrete, function (value) {
        //  Remove o cookie
...
function addEventoAListEnviarLembrete(e) {
  if (e.currentTarget.checked == true) {
    ...
    var diasParaLembrete =
      e.target.offsetParent.parentNode.parentNode.children[3].children[1]
        .children[0].value;

    var cookieEvento = docCookies.getItem("CookieListaAgendaREAG");

    if (
      typeof cookieEvento != "undefined" &&
      cookieEvento != "" &&
      cookieEvento != null
    ) {
      // Se cookie já tiver valor
      var splitCookieEvento = cookieEvento.split(",");
      if ($.inArray(idEvento, splitCookieEvento) == -1) {
        // Se idEvento NÃO estiver no cookie
        splitCookieEvento.push(idEvento);
        docCookies.setItem("CookieListaAgendaREAG", splitCookieEvento);

        var cookieLembrete = docCookies.getItem(
          "CookieEnviarLembreteAgendaREAG"
        ); //  Adiciona o cookie
        ...
        else alert("This item has already been added");
      }
    } else {
      docCookies.setItem("CookieListaAgendaREAG", idEvento);
      docCookies.setItem(
        "CookieEnviarLembreteAgendaREAG",
        idEvento + "&D" + diasParaLembrete
      );
...
function MarcarLembrete() {
  ...
  var getCookieLembrete = docCookies
    .getItem("CookieEnviarLembreteAgendaREAG")
    .split(",");
